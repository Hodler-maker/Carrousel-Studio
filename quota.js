...
(function (global) {
  ...
  // Pas encore de système de comptes/paiement centralisé côté serveur pour
  // TOUTES les pages : quand une page a branché le vrai forfait Firestore
  // (via window.csCurrentPlanId, voir carrousel.html/editor.html/visuel.html),
  // on s'en sert en priorité. Sinon on retombe sur l'ancien repère localStorage
  // (qui n'est en pratique jamais positionné → défaut "freemium").
  function getPlan() {
    if (typeof global.csCurrentPlanId === 'string' && global.csCurrentPlanId) {
      return global.csCurrentPlanId === 'genesis' ? 'freemium' : global.csCurrentPlanId;
    }
